...
export default function CandidateDashboard() {
  ...
  useEffect(() => {
    if (currentUser && preloaded?.ready) {
      processData();
    }
  }, [currentUser, preloaded?.ready]);

  const processData = () => {
    setLoading(true);
    setError(null);
    try {
      const allJobs = preloaded?.jobs || [];
      let userApplications = [...(preloaded?.userCandidates || [])];
      const userInterviews = preloaded?.userInterviews || [];
      ...
      if (currentUser?.email) {
        ...
        const richKey = `candidate_apps_${currentUser.email}`;
        ...
        const simpleKey = `applications_${currentUser.email}`;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-400 text-lg">
            Welcome back, {currentUser?.displayName || currentUser?.email?.split('@')[0] || 'Candidate'}
          </p>
        </motion.div>
