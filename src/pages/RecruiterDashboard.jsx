...
export default function RecruiterDashboard() {
  ...
  useEffect(() => {
    ...
    if (preloaded?.recruiterStats) {
      ...
      setLoading(false);
    } else if (preloaded?.ready) {
      // Preload finished but no recruiterStats — fetch manually
      fetchDashboardData();
    }
  }, [preloaded?.recruiterStats, preloaded?.ready]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const res = await api.get(`/recruitment/jobs/recruiter-stats/?email=${encodeURIComponent(email)}`);
      const data = res.data;
